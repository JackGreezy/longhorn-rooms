...
export default function ControlPanel({ view, setView }: ControlPanelProps) {
  return (
    <div className="w-72 p-4 border-r border-gray-300">
      ...
      <button
        onClick={() => setView("map")}
        className={`w-full mb-2 px-4 py-2 text-white rounded ${view === "map" ? "bg-orange-600" : "bg-orange-400"}`}
      >
        Map View
      </button>
      <button
        onClick={() => setView("list")}
        className={`w-full px-4 py-2 text-white rounded ${view === "list" ? "bg-orange-600" : "bg-orange-400"}`}
      >
        List View
      </button>
    </div>
